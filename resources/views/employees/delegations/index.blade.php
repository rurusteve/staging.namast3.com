            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header card-header-primary card-header-icon">
                        ...
                        <h3 class="card-title">Delegation List</h3>
                    </div>
                    <div class="card-body">
                        <table class="table responsive table-responsive-lg table-striped display" id="example">
                            ...
                            @foreach($delegations as $index => $delegation)
                                <tr>
                                    ...
                                    <td>{{ ucwords(strtolower($delegation->engagementType)) }}</td>
                                    <td>{{ ucwords(strtolower($delegation->engagementPeriod)) }}</td>
                                    <td>{{ ucwords(strtolower($delegation->groupName)) }}</td>
                                    <td>{{ $delegation->pic }}</td>
                                    <td>{{ $delegation->institusi }}</td>
                                    <td>{{ $delegation->branch }}</td>
                                    <td>
                                        <form action="{{ URL::to('/team/delegations/'.$delegation->id) }}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                            <button onclick="return confirm('Are you sure?')"
                                                    ...
                                                    data-target="#confirmDelete" data-title="Delete Delegation"
                                                    data-message='Are you sure you want to delete this delegation ?'>
                        ...
                        <a class="btn btn-success" style="margin-right: 5px;"
                           href="{{ URL::to('/team/delegations/create') }}">
                            <i class="fas fa-thumbtack"></i> Add Delegation
                        </a>
