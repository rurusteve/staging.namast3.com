                    ...
                    <div class="card-body">
                        <table class="table responsive table-responsive-lg table-striped display" id="example">
                            ...
                            @foreach($groups as $index => $group)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ ucwords(strtolower($group->division)) }}</td>
                                    <td>{{ ucwords(strtolower($group->name)) }}</td>
                                    <td>{{ ucwords(strtolower($group->code)) }}</td>
                                    <td>
                                        <form action="{{ URL::to('/team/groups/'.$group->id) }}" method="POST">
                                            @method('DELETE')
                                            @csrf
                                            <button onclick="return confirm('Are you sure?')"
                                                    ...
                                                    data-target="#confirmDelete" data-title="Delete group"
                        ...
                        <a class="btn btn-success" style="margin-right: 5px;"
                           href="{{ URL::to('/team/groups/create') }}">
                            <i class="fas fa-thumbtack"></i> Add Group
                        </a>
