            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        Approving Leave Request by {{ucwords(strtolower($employees->nama))}}
                        , {{ date('d M y', strtotime($leaverequestdetails->tanggalmulaicuti)) }} -
                        {{ date('d M y', strtotime($leaverequestdetails->tanggalakhircuti)) }}
                    </div>
                    <div class="card-body">
                        <form action="{{ URL::to('/adminrequestleavelist/approved/'.$nip.'/'.$id) }}" method="POST">
                            {{ csrf_field() }}

                            <div class="">
                                <label style="display: none;" for="keteranganapproval"
                                       class="col-md-4 col-form-label text-md-right">{{ __('Approval Message') }}</label>

                                <div class="col-md-6">
                                    <div class="alert-heading">Approval Message</div>
